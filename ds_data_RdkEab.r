ds_data_RdkEab <- function(dat, target_dir, spp, phstrip) {
    # THIS VERSION CALCULATES DISTANCE USING RDK'S METHODS AND EAB'S METHOD. THE DIFFERENCE IS NEGLIDGABLE.
    ...
    names(effort_summary) <-
        c("FILEID", "LEGNO2", "YEAR", "MONTH", "DAY", "Effort.rdk","Effort.eab")
    ...
    for (i in 1:length(ufid)) {
        ...
        for (j in 1:length(uLegno)) {
            # for each LEGNO2 flown in this FILEID
            ...
            for (k in 1:numRecs) {
                ...
                if (tmp.tmp.dat$OnOff.Effort[k] == 1 &
                    ...
                    #using rdk's equation
                    tmp.tmp.dat$pt2pt.effort.rdk[k] <-
                        dist.rdk(
                    ...
                    dat$pt2pt.effort.rdk[I[J[k]]] <-
                        dist.rdk(dat$LATITUDE[I[J[k]]],
                                 dat$LATITUDE[I[J[k + 1]]],
                                 dat$LONGITUDE[I[J[k]]],
                                 dat$LONGITUDE[I[J[k + 1]]])
                    
                    #using eab's equation
                    tmp.tmp.dat$pt2pt.effort.eab[k] <-
                        fn.grcirclkm(
                            tmp.tmp.dat$LATITUDE[k],
                            tmp.tmp.dat$LONGITUDE[k],
                            tmp.tmp.dat$LATITUDE[k + 1],
            ...
            effort_summary$Effort.rdk[ctr] <-
                sum(tmp.tmp.dat$pt2pt.effort.rdk, na.rm = TRUE)
            effort_summary$Effort.eab[ctr] <-
                sum(tmp.tmp.dat$pt2pt.effort.eab, na.rm = TRUE)
            
            # store distance for each LEGNO2 in the original dataset
            dat$Effort.rdk[I[J]] <-
                sum(tmp.tmp.dat$pt2pt.effort.rdk, na.rm = TRUE)
            dat$Effort.eab[I[J]] <-
                sum(tmp.tmp.dat$pt2pt.effort.eab, na.rm = TRUE)
    ...
    names(effort_summary_reduced) <- c("LEGNO2", "Effort.rdk","Effort.eab")
    for (i in 1:length(uLegno)) {
        effort_summary_reduced$LEGNO2[i] = uLegno[i]
        
        I <- which(effort_summary$LEGNO2 == uLegno[i])
        effort_summary_reduced$Effort.rdk[i] = sum(effort_summary$Effort.rdk[I])
        effort_summary_reduced$Effort.eab[i] = sum(effort_summary$Effort.eab[I])
    }
    ...
    for (i in 1:nrow(effort_summary_reduced)) {
        ...
        if (length(I) > 0) {
            ...
            dat.spp$Effort.rdk[I] = effort_summary_reduced$Effort.rdk[i]
            dat.spp$Effort.eab[I] = effort_summary_reduced$Effort.eab[i]
        } else {
            ...
            dat.spp$Effort.rdk[nrow(dat.spp)]       <-
                effort_summary_reduced$Effort.rdk[i]
            dat.spp$Effort.eab[nrow(dat.spp)]       <-
                effort_summary_reduced$Effort.eab[i]
